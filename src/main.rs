...
fn main() {
    ...
    if config.count {
        ...
        for password in &passwords {
            ...
            total_count += generate_variations(password, config.max_distance, config.transposition_distance, config.max_transpositions).iter().count() as u64;
        }
        println!("{}", total_count);
    } else {
        ...
        for password in &passwords {
            ...
            all_variations.extend(generate_variations(password, config.max_distance, config.transposition_distance, config.max_transpositions));
        }
